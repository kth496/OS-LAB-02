 ...
 *
 * You can compare single thread result, coarse grained result and fine grained
 *result.
 */
...
static void print_result(lab2_tree *tree, int num_threads, int node_count,
                         int is_sync, int op_type, double time) {
        char *cond[] = {"fine-grained BST  ", "coarse-grained BST",
                        "single thread BST"};
...
void bst_test(int num_threads, int node_count) {
        ...
        lab2_tree *tree;
        lab2_node *node;
        struct timeval tv_insert_start, tv_insert_end, tv_delete_start,
            tv_delete_end, tv_start, tv_end;
        ...
        for (i = 0; i < num_threads; i++) {
                ...
                pthread_create(&threads[i].thread, NULL, thread_job_insert,
                               (void *)th_arg);
        }
        ...
        for (i = 0; i < num_threads; i++) {
                ...
                pthread_create(&threads[i].thread, NULL, thread_job_delete,
                               (void *)th_arg);
        }
        ...
        print_result(tree, num_threads, node_count, is_sync, LAB2_OPTYPE_DELETE,
                     exe_time);
        lab2_tree_delete(tree);
