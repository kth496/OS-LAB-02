...
void lab2_bonus_usage(char *cmd) {
        ...
        printf("    -i: iteration count, must be bigger than 0 ( e.g. 100000 ) "
               "\n");
        printf("    -s: sync mode setting (e.g. s : your implemted spin lock , "
               "o : original) \n");
}
...
int your_spinlock_test(int num_threads, int num_iterations, int is_sync) {
        ...
        for (i = 0; i < num_threads; i++) {
                res = pthread_create(&pthreads[i], NULL, add_shared_variable,
                                     (void *)&arg);
                if (res == LAB2_ERROR) {
                        printf(
                            " Error: _perf_metadata - pthread_create error \n");
                        goto TEST_ERROR;
                }
        }
